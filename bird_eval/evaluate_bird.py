...
def evaluate_sqls_parallel(db_files, questions, pred_sqls, ground_truth_sqls, num_cpus=1, timeout=5):
...
def execute_sqls_parallel(db_files, sqls, num_cpus=1, timeout=5):
    pool = mp.Pool(processes=num_cpus)
...
def mark_invalid_sqls(db_files, sqls):
    ...
    execution_results = []
    execute_sqls_parallel(db_files, sqls, num_cpus=20, timeout=60)
