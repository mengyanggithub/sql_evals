...
def parse_response_deprecated(response):
    pattern = r"```sql\s*(.*?)\s*```"
...
def extract_sql_from_markdown(markdown_text):
    ...
    # 使用re.DOTALL标志来匹配跨越多行的内容
    matches = re.findall(pattern, markdown_text, re.DOTALL)

    # 去除每个匹配结果的前后空白字符
    json_strings = matches[-1].strip()

    return json_strings

def extract_sql_from_markdown_2(markdown_text):
    # 定义正则表达式模式
    pattern = r'```sql(.*?)```'
...
def parse_response(response):
    try:
        response = extract_sql_from_markdown(response)
    except:
        try:
            response = extract_sql_from_markdown_2(response)
        except:
            print("Cannot parse response: ", response)
            response = ""
            return response
    return response
...
if __name__ == "__main__":
    ...
    max_model_len = 18192  # used to allocate KV cache memory in advance
    max_input_len = 8192
    max_output_len = 10000  # (max_input_len + max_output_len) must <= max_model_len

    print("max_model_len:", max_model_len)
    print("temperature:", opt.temperature)
    sampling_params = SamplingParams(
        temperature=opt.temperature, top_p=0.9, max_tokens=max_output_len, n=opt.n, stop_token_ids=stop_token_ids
    )
    ...
    if opt.parallel_generation:
        ...
        # else:
        vllm_tp_size = 4
        if opt.tensor_parallel_size > vllm_tp_size:
        ...
        model_params = {
            "model":opt.pretrained_model_name_or_path,
            "tokenizer":opt.pretrained_model_name_or_path,
            "tensor_parallel_size":vllm_tp_size,
            "dtype":"auto",
            "max_seq_len_to_capture":32000,  # 设置最大序列长度
            "trust_remote_code": True
        }
...
Output Format:
Show your work in <think> </think> tags. And return the final SQLite SQL query in <sql> </sql> tags, for example <think>your thought process</think> <sql>SELECT AVG(rating_score) FROM movies</sql>.
Take a deep breath and think step by step to find the correct SQL query."""
    ...
    if opt.parallel_generation:
        ...
        outputs = []
        for batch in batch_iterable(chat_prompts, batch_size=128):
            sub_outputs = ray.get([llm.generate.remote(batch, sampling_params) for llm in llm_list])
            outputs.extend(sum(sub_outputs, []))
    else:
        outputs = []
        for batch in batch_iterable(chat_prompts, batch_size=128):
            sub_outputs = llm.generate(batch, sampling_params)
